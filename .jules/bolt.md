...
**Learning:** Filtering arrays three times within a React render (like `csvUpload.queue.filter(...)` called inline in multiple JSX nodes) triggers unnecessary O(N) array traversals on every render update.
**Action:** Extract repeated array filtering methods from directly inside TSX structures into a single `useMemo` hook to run exactly once per dependencies change.
